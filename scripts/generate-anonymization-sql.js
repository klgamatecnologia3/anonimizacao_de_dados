// Gera um script SQL de anonimização para o ambiente de homologação.
// O script conecta no Postgres, detecta colunas com padrão sensível, amostra até 20 valores
// por coluna para estimar tamanhos médios e escreve UPDATEs com dados fictícios compatíveis.
...
const OUTPUT_SQL_PATH = path.resolve(process.env.ANON_SQL_OUTPUT || 'sql/anonymize-homologation.sql');
...
function buildJsonExpression(meta, castType) {
  const payload = {
    anon: true,
    origem: 'script_sql',
...
function isTextual(meta) {
  const type = stripLength(meta.data_type);
  return ['text', 'character varying', 'character', 'bpchar', 'json', 'jsonb', 'uuid'].includes(type);
}
...
function getExclusionPredicate(tableName, availableColumns) {
  ...
  if (normalizeName(tableName) === 'clientes') {
    if (columns.has('ID')) {
...
function resolveStrategy(meta) {
  ...
  if (isBytea(meta)) {
    return { category: 'bytea_null', expression: 'NULL' };
  }
  ...
  if (ENABLE_JSON_ANONYMIZATION && (name.includes('json') || dataType === 'json' || dataType === 'jsonb')) {
    return { category: 'json', expression: buildJsonExpression(meta, castType) };
  }

  if (name.includes('nasc')) {
    return { category: 'data', expression: buildDateExpression(meta.column_name, castType) };
  }
...
function buildHeader(strategySummary) {
  ...
  const lines = [
    '-- Script SQL de anonimização gerado automaticamente.',
    `-- Schema alvo: ${TARGET_SCHEMA}`,
    `-- Gerado em: ${generatedAt}`,
    '-- Regras principais:',
    '-- - preserva a compatibilidade geral de tipos;',
    '-- - usa amostra de até 20 valores por coluna para estimar comprimento;',
    '-- - evita atualizar IDs e colunas booleanas;',
    '-- - preserva datas em geral; anonimiza apenas datas pessoais evidentes, como nascimento;',
    '-- - define bytea como NULL quando entrar no escopo da anonimização;',
    '-- - preserva apenas o registro ID = 1 da tabela clientes.',
    '-- Sugestoes para revisar antes de executar:',
    '-- - emails internos que devam ser preservados;',
    '-- - anexos fisicos fora da base;',
    '-- - campos textuais livres com regras de negocio especificas.',
    '',
    'begin;',
    '',
...
async function main() {
  ...
  try {
    ...
    const summary = [...strategyCounts.entries()].sort((left, right) => left[0].localeCompare(right[0]));
...
main().catch((error) => {
  console.error('Erro ao gerar SQL de anonimização:', error.message);
  process.exit(1);
});
